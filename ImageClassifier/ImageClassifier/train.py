...
def main():
    ...
    train_loader, validation_loader, test_loader, train_dataset = Common.load_data(data_dir)

    print("Setting up model architecture...")
    model, criterion, optimizer = Common.model_setup(architecture, dropout, hidden_units, learning_rate, hardware)

    print("Training model...")
    Common.train_network(train_loader, validation_loader, model, criterion, optimizer, epochs, print_every, hardware)

    print("Validating testing accuracy...")
    Common.test_accuracy(model, test_loader, hardware)

    print("Saving model to disk...")
    Common.save_checkpoint(model, train_dataset.class_to_idx, path, architecture, hidden_units, dropout, learning_rate)
